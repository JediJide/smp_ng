...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.lexicon.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.lexicons.update", [$lexicon->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('therapy_area') ? 'is-invalid' : '' }}" name="therapy_area_id" id="therapy_area_id" required>
                    @foreach($therapy_areas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('therapy_area_id') ? old('therapy_area_id') : $lexicon->therapy_area->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('preferred_phrase') ? 'is-invalid' : '' }}" name="preferred_phrase" id="preferred_phrase">{!! old('preferred_phrase', $lexicon->preferred_phrase) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('guidance_for_usage') ? 'is-invalid' : '' }}" name="guidance_for_usage" id="guidance_for_usage">{!! old('guidance_for_usage', $lexicon->guidance_for_usage) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('non_preferred_terms') ? 'is-invalid' : '' }}" name="non_preferred_terms" id="non_preferred_terms">{!! old('non_preferred_terms', $lexicon->non_preferred_terms) !!}</textarea>
